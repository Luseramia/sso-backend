...
import { ocrClient, type ImageRequest, type BatchImageRequest, type MultiImageRequest, type OCRResult, type BatchOCRResult, type PDFStatementRequest, type StatementResult } from "./grpc-client";
    ...
    app
        ...
        .post(
            ...
            async ({ body, set }) => {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    ocrClient.ProcessStatement(request, (error: any, response: StatementResult) => {
                        if (error) {
                            console.error("gRPC Error:", error);
                            set.status = 500;
                            resolve({ error: error.message || "Internal Server Error" });
        ...
        .get(
            ...
            async ({ query, set }) => {
                ...
                try {
                    const data = await bankTransactionService.getByDateRange({ year, month, day });
                    return data;
                } catch (error: any) {
